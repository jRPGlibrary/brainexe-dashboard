...
function renderLogs() {
  ...
  if (state.logs.length > 100) {
    ...
    logsPagination.filters.type = _logTypeFilter;
    sec.innerHTML = `
      <div class="card">
        <div class="card-header">
          <div>
            <div class="card-title">Logs en direct (Pagination)</div>
            <div class="card-subtitle" id="logs-count">${state.logs.length} événements</div>
          </div>
          <div style="display: flex; gap: 6px;">
            <button class="btn btn-sm btn-ghost" onclick="state.logs = []; logsPagination.renderPage()">🗑 Vider</button>
          </div>
        </div>
        ${logsPagination.renderFilters()}
        <div id="logs-stream"></div>
      </div>
    `;

    logsPagination.currentPage = 0;
    logsPagination.renderPage();
  } else {
    sec.innerHTML = `
      <div class="card">
        <div class="card-header">
