  ...
  setTimeout(async () => {
    try {
      ...
      const _alreadyReplied = [...fetched.values()].some(m =>
        m.author?.id === _botId &&
        m.createdTimestamp > _twoHAgo &&
        m.createdTimestamp > message.createdTimestamp &&
        (m.reference?.messageId === message.id || m.mentions?.users?.has(message.author.id))
      );
      if (_alreadyReplied) {
  ...
  setTimeout(async () => {
    try {
      ...
      const _alreadyRepliedS = [...msgs.values()].some(m =>
        m.author?.id === _botIdS &&
        m.createdTimestamp > _twoHAgoS &&
        m.createdTimestamp > lastMsg.createdTimestamp &&
        (m.reference?.messageId === lastMsg.id || m.mentions?.users?.has(lastMsg.author.id))
      );
      if (_alreadyRepliedS) {
