...
function registerRoutes(app) {
  ...
  // Backup
  app.post('/api/backup', async (req, res) => { try { const state = await readGuildState(); const snapshot = { state, botConfig: shared.botConfig, createdAt: new Date().toISOString() }; const fn = `backup_${new Date().toISOString().replace(/[:.]/g, '-')}.json`; fs.writeFileSync(fn, JSON.stringify(snapshot, null, 2), 'utf8'); pushLog('SYS', `Backup : ${fn}`, 'success'); auditLog('backup.create', { name: fn }); res.json({ ok: true, filename: fn }); } catch (e) { res.status(500).json({ ok: false, error: e.message }); } });
